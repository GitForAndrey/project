...
export const MatchupListItem = ({ item }) => {
  const { allTags } = useSelector(state => state.athleteTags);
  const teamTags1 = useSelector(state => state.matchup.activeTags).reduce(
    (acc, el) => {
      if (el.team_id == item.team_1_id) {
  ...
  const teamTags2 = useSelector(state => state.matchup.activeTags).reduce(
    (acc, el) => {
      if (el.team_id == item.team_2_id) {
        acc[el.tag_id] = (acc[el.tag_id] || 0) + 1;
      }
      return acc;
    },
  ...
  return (
    <View style={styles.container}>
      ...
      <ScrollView
        ...
        showsHorizontalScrollIndicator={false}
        style={{ marginTop: 10 }}>
        {renderTags(teamTags2, allTags)}
      </ScrollView>
    </View>
  );
};
export default MatchupItem = React.memo(MatchupListItem);
